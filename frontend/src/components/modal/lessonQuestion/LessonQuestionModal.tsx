...
const LessonQuestionModal: React.FC<LessonQuestionModalProps> = ({
  ...
  useEffect(() => {
    ...
    socket.on("question:updated", (question: ApiQuestion) => {
      console.log("[LessonQuestionModal] question:updated 이벤트 수신:", question);
      setQuestions((prev) =>
    ...
    // GPT 답변 생성 시
    socket.on("question:answer", (data: { question_id: string; answer: string; question: ApiQuestion }) => {
      ...
      const checkAnswer = async (questionId: string, retries = 10) => {
        ...
        setTimeout(async () => {
          try {
            ...
            const response = await getClassQuestions(lectureId!, classId!);
            const updatedQuestion = response.questions?.find((q) => q._id === questionId);
      ...
      <div className="space-y-4">
        ...
        <div className="flex gap-6">
            ...
            <div ref={pdfContainerRef} className="flex-1 bg-gray-50 border-t border-gray-200">
              {pages && pages.length > 0 ? (
                ...
                <div className="w-full min-h-[600px] flex flex-col items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 p-8">
                  ...
                  <p className="text-gray-400 text-sm mt-2">PDF 파일이 업로드되면 여기에 표시됩니다</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="w-80 space-y-4 border border-gray-200 bg-white p-4">
            <div className="flex items-center justify-between">
              <h4 className="text-lg font-semibold text-gray-900">질문 및 답변</h4>
              <div className="flex items-center space-x-1">
                <button
                  onClick={() => setSortBy("upvote")}
                  className={clsx("px-2 py-1 text-xs border transition-colors duration-200", {
                    "bg-blue-100 text-blue-700 border-blue-300": sortBy === "upvote",
                    "text-gray-500 hover:bg-gray-100 border-gray-300": sortBy !== "upvote",
                  })}
                >
                  UPVOTE순
                </button>
                <button
                  ...
                  className={clsx("px-2 py-1 text-xs border transition-colors duration-200", {
                    "bg-blue-100 text-blue-700 border-blue-300": sortBy === "latest",
                    "text-gray-500 hover:bg-gray-100 border-gray-300": sortBy !== "latest",
                  })}
                >
                  최신순
                </button>
              ...
              <div className="border border-gray-200 rounded-lg p-3 bg-gray-50">
                ...
                <div className="flex justify-end space-x-2 mt-2">
                  ...
                  <button
                    ...
                    className={clsx("px-4 py-1 bg-blue-600 text-white text-sm rounded", {
                      "opacity-50 cursor-not-allowed": !newQuestionText.trim() || isCreatingQuestion,
                      "hover:bg-blue-700": newQuestionText.trim() && !isCreatingQuestion,
                    })}
                  >
            ...
            <div className="max-h-96 overflow-y-auto space-y-3 pr-2">
              {isLoadingQuestions ? (
                <div className="text-center text-gray-500 py-4">질문을 불러오는 중...</div>
              ) : sortedQuestions.length === 0 ? (
                <div className="text-center text-gray-500 py-4">아직 질문이 없습니다.</div>
              ) : (
                sortedQuestions.map((question) => {
                  const isOpen = replyingTo === question._id;
                  const isUpvoted = userInfo && question.upvoted_by?.includes(userInfo.id);
                  const hasAnswer = !!question.answer;

                  return (
                    <div key={question._id} className="border border-gray-200 bg-white rounded-lg">
                      <div className="p-3">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center space-x-2 mb-1">
                              ...
                              <span className="text-xs text-gray-500">
                                {formatTimeAgo(question.created_at || question.timestamp)}
                              </span>
                              {question.page && (
                            ...
                            <p className="text-sm text-gray-900 mb-2">Q. {question.text}</p>
                              ...
                              <button
                                ...
                                className={clsx(
                                  "flex items-center space-x-1 px-2 py-1 rounded text-xs transition-colors",
                                  {
                                    ...
                                    "bg-gray-100 text-gray-600 hover:bg-gray-200": !isUpvoted,
                                  }
                                )}
                              >
                        ...
                        {hasAnswer && (
                          <div className="mt-3 pt-3 border-t border-gray-200 space-y-3">
                            ...
                            <div className="flex items-start space-x-2">
                              ...
                              <div className="flex-1">
                                <div className="flex items-center space-x-2 mb-1">
                                  <span className="text-xs font-medium text-gray-900">AI 레큐</span>
                                </div>
                                <p className="text-xs text-gray-700">A. {question.answer}</p>
                              </div>
                            </div>
                          </div>
                        )}
                          ...
                          <div className="mt-3 pt-3 border-t border-gray-200">
                            ...
                            <div className="flex justify-end space-x-2 mt-2">
                              ...
                              <button
                                ...
                                className={clsx("px-4 py-1 bg-blue-600 text-white text-xs rounded", {
                                  "opacity-50 cursor-not-allowed": !replyText[question._id]?.trim(),
                                  "hover:bg-blue-700": replyText[question._id]?.trim(),
                                })}
                              >
                                답변 달기
                              </button>
